...
public class CheeseListFragment extends Fragment {
    ...
    public static class SimpleStringRecyclerViewAdapter
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            ...
            holder.mImageView.setTransitionName(position + "");
            holder.mView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(activity, CheeseDetailActivity.class);
                    intent.putExtra("position", position);
                    intent.putExtra(CheeseDetailActivity.EXTRA_NAME, holder.mBoundString);
                    activity.startActivity(intent, ActivityOptions.makeSceneTransitionAnimation(activity,
                            holder.mImageView, holder.mImageView.getTransitionName()).toBundle());
                }
            });

            Glide.with(holder.mImageView.getContext())
                    .load(Cheeses.getRandomCheeseDrawable(position))
                    .fitCenter()
